...
- Home `/` is the **B2B landing** for the CRM service. It renders HTML/CSS saved in the `Setting` table under keys `b2b-landing-css` and `b2b-landing-body` (helpers: `getB2BLandingContent`/`setB2BLandingContent` in `lib/b2b-landing.server.js`). The defaults live in `lib/b2b-landing.js` (`DEFAULT_B2B_LANDING_CSS` and `DEFAULT_B2B_LANDING_BODY`) and use the classic **Mundo theme** (dark teal, hero slider, marquee, etc.). The page is `force-dynamic` so admin edits show immediately.
...
- The admin `B2BLandingEditor.jsx` edits the B2B landing through **Visual** (inline contenteditable on the preview iframe), **HTML** and **CSS** tabs. Saving does `PUT /api/b2b-landing` with `{ css, body }`.
...
- The seller editor preview uses a **same-origin iframe** to `/p/{slug}?preview=1` and communicates with the parent via `postMessage`.
