export const B2B_MOBILE_CSS = `
  @media (max-width: 768px) {
    ...
    .hero, .hero-slide, .hero-slider { min-height: 0; }
    .hero-slider { width: 100%; overflow: hidden; }
    .hero-slide { width: 100%; min-width: 100%; flex: 0 0 100%; padding: 2.5rem 0 4.5rem; }
    .hero-slide .container, .seller-card, .coverage-layout, .ego-grid, .lead-grid, .footer-layout { grid-template-columns: 1fr; }
    .hero-slide .container, .hero-content, .hero-image-container, .hero-card { min-width: 0; }
    .hero-slide .container { width: 100%; }
    .hero-content { width: 100%; max-width: none; text-align: center; }
    .hero-seller-mobile-photo { display: block; width: 96px; height: 96px; margin: 0 auto 1rem; position: relative; overflow: hidden; border: 3px solid var(--color-secondary); border-radius: 50%; box-shadow: 0 8px 24px rgba(0, 0, 0, .35); }
    ...
    .hero-card-title { font-size: clamp(1.35rem, 7vw, 2rem); line-height: 1.15; }
