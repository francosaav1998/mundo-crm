const DEFAULT_LANDING_CONTENT = {
    ...
    slides: [
      {
        ...
        title: "La velocidad que mueve",
        titleHighlight: "tu mundo",
        description: "Conecta todos tus dispositivos con fibra óptica, instalación rápida y atención personalizada.",
        ctaPrimary: "Ver planes",
        ctaSecondary: "Consultar cobertura",
        icon: "bi-lightning-charge-fill",
      },
      {
        badge: "Fibra óptica Mundo",
        title: "Más velocidad para",
        titleHighlight: "hacerlo todo",
        description: "Streaming, teletrabajo y gaming sin pausas con una conexión estable para toda la familia.",
        ctaPrimary: "Quiero conectarme",
        ctaSecondary: "Evaluar cobertura",
        icon: "bi-wifi",
      },
      {
        badge: "Atención personalizada",
        title: "Tu próximo plan empieza",
        titleHighlight: "con una conversación",
        description: "Un ejecutivo Mundo te ayuda a encontrar la alternativa ideal para tu hogar.",
        ctaPrimary: "Hablar con un ejecutivo",
  ...
  header: {
    ...
    marqueeItems: [
      ...
      { icon: "bi-tools", text: "Instalación express en 24 a 48 horas" },
      { icon: "bi-headset", text: "Atención personalizada de lunes a domingo" },
      { icon: "bi-check-circle-fill", text: "Evaluación de cobertura en 5 minutos" },
    ],
    navLinks: [
...
export function isValidPlanPrice(price) {
  const digits = String(price ?? "").replace(/[^0-9]/g, "");
  return digits.length > 0 && Number(digits) > 0;
}

export function getMergedPlans(plans, overrides) {
  if (!overrides || overrides.length === 0) return plans.filter((plan) => isValidPlanPrice(plan.price));
  ...
  const merged = plans
    ...
    .filter((p) => p.sellerActive !== false && isValidPlanPrice(p.price))
