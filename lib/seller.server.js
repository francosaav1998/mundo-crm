...
export async function findOrCreateSellerForUser(user, companySlug) {
  ...
  } else if (company && seller.companyId !== company.id) {
    // Sincroniza la compañía elegida en el registro con el seller actual.
    // Esto corrige casos donde el seller quedó creado con la compañía por defecto.
    seller = await prisma.seller.update({
      where: { id: seller.id },
