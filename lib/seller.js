...
export function getDefaultBio(gender, companyName = "Mundo") {
  const labels = getSellerLabels(gender);
  return `Como ${labels.yourExecutive} comercial especializada de ${companyName}, te ayudo a gestionar tu contrato de forma rápida y transparente. Olvídate de largas esperas en call centers. Analizo la cobertura de tu sector en minutos y agendo tu instalación en tiempo récord.`;
}

export function getDefaultFooterText(gender, companyName = "Mundo") {
  const labels = getSellerLabels(gender);
  return `${labels.executiveCapitalized} comercial autorizada de ${companyName}. Gestión ágil, directa y transparente de tus planes de internet fibra, televisión digital y telefonía móvil.`;
}

export function getShortBio(gender) {
  const labels = getSellerLabels(gender);
  return `Como ${labels.yourExecutive} comercial especializada, te ayudo a gestionar tu contrato de forma rápida y transparente.`;
}
...
export function getWhatsAppUrl(text, phone = SELLER_CONFIG.phone) {
  ...
  return normalizedPhone
    ? `${baseUrl}?phone=${normalizedPhone}&text=${encodedText}`
